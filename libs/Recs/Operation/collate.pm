...
use Recs::Aggregator::Last;
use Recs::Aggregator;
use Recs::DomainLanguage;
use Recs::DomainLanguage::Executor;
use Recs::DomainLanguage::Library;
...
sub build_dlkey {
   ...
   $dlkeys_ref->{$name} = Recs::DomainLanguage::Snippet->new($string)->evaluate_as('VALUATION');
}
...
sub build_dlaggregator {
   ...
   $dlaggregators_ref->{$name} = Recs::DomainLanguage::Snippet->new($string)->evaluate_as('AGG');
}
