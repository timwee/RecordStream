package Recs::DomainLanguage::Value;

use strict;
use warnings;

use Recs::Aggregator::Internal::Constant;
use Recs::DomainLanguage::Snippet;
...
sub add_possibility
{
    ...
    if($type eq "SCALAR")
    {
        $this->add_possibility('VALUATION', Recs::DomainLanguage::Valuation::KeySpec->new($value));
        $this->add_possibility('SNIPPET', Recs::DomainLanguage::Snippet->new($value));
    }
}
...
sub cast_valuation_or_die
{
    ...
    {
        return Recs::DomainLanguage::Valuation::Sub->new($obj);
    }

    if(blessed($obj) && $obj->isa('Recs::DomainLanguage::Value'))
    {
    ...
    if(blessed($obj) && $obj->isa('Recs::Aggregator::Aggregation'))
    {
        die "Aggregation found where valuation expected";
    }

    if(blessed($obj) && $obj->isa('Recs::DomainLanguage::Valuation'))
    {
        return $obj;
    }
...
sub cast_agg_or_die
{
    ...
    return Recs::Aggregator::Internal::Constant->new($obj);
}
