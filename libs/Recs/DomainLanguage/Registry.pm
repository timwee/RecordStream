package Recs::DomainLanguage::Registry;
...
sub evaluate
{
    ...
    for my $arg (@raw_args)
    {
        if(blessed($arg) && $arg->isa('Recs::DomainLanguage::Value'))
        {
            push @value_args, $arg;
            next;
        }
        my $value = Recs::DomainLanguage::Value->new("<raw object>");
        my $done = 0;
        if(blessed($arg) && $arg->isa('Recs::DomainLanguage::Valuation'))
        {
            $value->add_possibility('VALUATION', $arg);
            $done = 1;
        }
        if(blessed($arg) && $arg->isa('Recs::Aggregator::Aggregation'))
        {
        ...
        push @value_args, Recs::DomainLanguage::Value->new_from_scalar($arg);
    }
    ...
    my $ret = Recs::DomainLanguage::Value->new($token . "(" . join(", ", map { $_->get_description() } @value_args) . ")");

    for my $result (@results)
    {
        if(blessed($result) && $result->isa('Recs::DomainLanguage::Value'))
        {
        ...
        my $done = 0;
        if(blessed($result) && $result->isa('Recs::DomainLanguage::Valuation'))
        {
            $ret->add_possibility('VALUATION', $result);
            $done = 1;
        }
        if(blessed($result) && $result->isa('Recs::Aggregator::Aggregation'))
        {
